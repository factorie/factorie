...
class TestTransitionBasedParser {
  ...
  // TODO fix this!!
  //val testFileName = "/Users/strubell/Documents/research/test_input_parse"//this.getClass.getResource("test_input").getPath()
  val testFileName = "/Users/strubell/Documents/research/parse-nonulls-errors-data"
  ...
  def testDepToken() = {
	...
	val goldDoc = LoadOntonotes5.fromFilename(testFileName).head
	val testSentences = testDoc.sentences
	val goldSentences = goldDoc.sentences
	
	testSentences.zip(goldSentences).foreach(sentences => {
      val parseTree = sentences._2.attr[ParseTree]
      parseTree.setParentsToTarget
      val parseDecisions = parser.generateDecisions(testSentences, parser.ParserConstants.TRAINING, numThreads)
      val sentenceWords = sentences._1.tokens.map(_.string)

      println(s"Sentence: ${sentenceWords.mkString(" ")}")
      println(s"Number of decisions: ${parseDecisions.size}")

      /* Check that the relations between tokens are correct */
      val sentenceTokens = parseDecisions.head.state.sentenceTokens
      sentenceTokens.takeRight(sentenceTokens.length-2).foreach(tok => {
...
//    	      Assert.assertEquals(s"Grandhead of token ``${tok.form}'' incorrect.", nullTokenString, thisGrandHead.form)
...
//    	  }
      ...
      parseDecisions.foreach(decision => {
