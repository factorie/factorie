...
class TestLexerTokenizer extends JUnitSuite with FastLogging {
  ...
  @Test def testSentenceSegmenter(): Unit = {
    ...
    DocumentAnnotatorPipeline(DeterministicLexerTokenizer, DeterministicSentenceSegmenter).process(d)
  ...
  private def runDeterministicLexerTokenizer(src: String): Iterable[Token] = {
    val d = new Document(src)
    DocumentAnnotatorPipeline(DeterministicLexerTokenizer).process(d).tokens
  }
...
//    noInference.foreach(printTokenizedSentences(_))
  ...
  @Test def testDeterministicLexerTokenizerUnclosedLine(): Unit = {
    ...
    val tokens = runDeterministicLexerTokenizer(unclosedLine)
  }
  ...
  @Test def testDeterministicLexerTokenizerLongLine(): Unit = {
    ...
    val tokens = runDeterministicLexerTokenizer(hugeLine)
  }
  
  @Test def testRegexTokenizer(): Unit = {
    assert(DeterministicLexerTokenizer("Washington D.C.").toSeq == Seq("Washington", "D.C."))
