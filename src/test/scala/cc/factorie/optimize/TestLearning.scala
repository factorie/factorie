...
class TestLearning {
  ...
  def testPseudolikelihood() {
    ...
    for ((ple, lle) <- plExamples.zip(llExamples)) {
      val localPLgrad = new LocalWeightsMapAccumulator(model.parameters.blankDenseCopy)
      ...
      val localLLgrad = new LocalWeightsMapAccumulator(model.parameters.blankDenseCopy)
