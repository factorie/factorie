...
object NounPhraser2 extends DocumentAnnotator {
  ...
  def postAttrs = Seq(classOf[NerPhrase], classOf[PronounPhrase])
  ...
  def getNerSpans(doc: Document): Seq[(String,TokenSpan)] = {
    ...
    for (s <- doc.sections;t <- s.tokens) {
      if (t.attr[NerLabel].categoryValue != "O") {
        ...
        if (attr(0) == "U") {
          val lab = attr(1)
          spans += (lab -> new NerPhrase(s, t.positionInSection, 1))
        } else if (attr(0) == "B") {
          val lab = attr(1)
          if(t.hasNext) {
            var lookFor = t.next
            while (lookFor.hasNext && lookFor.attr[NerLabel].categoryValue.matches("(I|L)-" + attr(1))) lookFor = lookFor.next
            spans += (lab -> new NerPhrase(s, t.positionInSection, lookFor.positionInSection - t.positionInSection))
          } else {
            spans += (lab -> new NerPhrase(s, t.positionInSection, 1))
          }
        }
        ...
        if ( t.string.length > 2 && !t.containsLowerCase && upperCase.findFirstIn(t.string).nonEmpty && (t.getNext ++ t.getPrev).exists(i => i.containsLowerCase)) {
          spans += ("ORG" -> new NerPhrase(s, t.positionInSection, 1))
        } else if (t.posLabel.categoryValue == "NNP") {
          spans += ("PER" -> new NerPhrase(s, t.positionInSection, 1))
        }
      }
    }
    spans.toSeq
  ...
  def getPronounSpans(doc: Document): Seq[TokenSpan] = {
    doc.tokens.filter(_.posLabel.isPersonalPronoun).map(t => new PronounPhrase(t.section, t.positionInSection, 1)).toSeq
  }
  ...
  def process(document: Document) = {
    val nerPhrases = getNerSpans(document).map(labelSpan => {
      ...
      val s = labelSpan._2
      val m = new NerPhrase(s.section, s.start, s.length, s.length-1)
    ...
    val pronounPhrases = getPronounSpans(document).map(span => {
      val m = new PronounPhrase(span.section, span.start, span.length, 0)
