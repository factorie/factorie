...
object NounPhraser3 extends DocumentAnnotator {
  ...
  private def nerSpans(doc: Document): Seq[NerPhrase] = {
    doc.attr[NounPhraseList].spansOfClass[NerPhrase]
  }

  private def NNPSpans(doc : Document) : Seq[NominalPhrase] = {
    ...
    (for(span <- spans) yield
      new NominalPhrase(span.head.section, span.head.positionInSection, span.last.positionInSection-span.head.positionInSection+1, span.last.positionInSection-span.head.positionInSection)).toSeq
  }

  // [Assumes personal pronouns are single tokens.]
  private def personalPronounSpans(doc: Document): Seq[PronounPhrase] = {
    (for (section <- doc.sections; s <- section.sentences;
           (t,i) <- s.tokens.zipWithIndex if isPersonalPronoun(t)) yield
        new PronounPhrase(section, s.start + i, 1,0)
      ).toSeq
  }

  private def getHeadTokenIdx(m: NounPhrase): Int = {
  ...
  private def nounPhraseSpans(doc: Document, nounFilter: Token => Boolean): Seq[NominalPhrase] = (
    ...
    for (section <- doc.sections; s <- section.sentences;
      ...
      if (FILTER_APPOS && (copularPhrase || apposition ||  parentIsNoun)) None
        ...
        val res = Some(new NominalPhrase(section, start, commaLength, headTokenIndexInSpan))
  ...
  private def removeSmallerIfHeadWordEqual(doc: Document, mentions: Seq[NounPhrase]): Seq[NounPhrase] =
    mentions
  ...
  private def dedup(mentions: Seq[NounPhrase]): Seq[NounPhrase] = {
    // Note: equality is only in the first set of arguments for case classes
    case class MentionStartLength(section:Section, start: Int, length: Int)(val mention: NounPhrase) {
      def this(mention: NounPhrase) = this(mention.section, mention.start, mention.length)(mention)
    }
  ...
  def process(doc: Document): Document = {
    ...
    var docMentions = new ArrayBuffer[NounPhrase]
    // NAM = proper noun, NOM = common noun, PRO = pronoun
    docMentions ++= nerSpans(doc)                       map(  m => {m.attr += new NounPhraseType(m,"NAM");m})
    docMentions ++= personalPronounSpans(doc)           map(  m => {m.attr += new NounPhraseType(m,"PRO");m})
    docMentions ++= nounPhraseSpans(doc, isCommonNoun)  map(  m => {m.attr += new NounPhraseType(m,"NOM");m})
    docMentions ++= nounPhraseSpans(doc, isProperNoun)  map(  m => {m.attr += new NounPhraseType(m,"NAM");m})
    docMentions ++= NNPSpans(doc)                       map(  m => {m.attr += new NounPhraseType(m,"NAM");m})
    ...
    doc.attr += (new NounPhraseList() ++= removeSmallerIfHeadWordEqual(doc, dedup(docMentions)).filter(mention => mention.attr[NounPhraseType] ne null).toSeq)

    doc
  }

  def prereqAttrs: Iterable[Class[_]] = Seq(classOf[parse.ParseTree])
  def postAttrs: Iterable[Class[_]] = Seq(classOf[NounPhraseList])
  override def tokenAnnotationString(token:Token): String = token.document.attr[NounPhraseList].filter(mention => mention.contains(token)) match { case ms:Seq[NounPhrase] if ms.length > 0 => ms.map(m => m.attr[NounPhraseType].categoryValue+":"+m.indexOf(token)).mkString(","); case _ => "_" }

  def addNerSpans[T <: NerLabel](doc: Document)(implicit m: Manifest[T]): Unit = {
    val nerSpans = doc.attr.getOrElseUpdate[NounPhraseList](new NounPhraseList)
    for (s <- doc.sections; t <- s.tokens) {
      if (t.attr[T].categoryValue != "O") {
        val attr = t.attr[T].categoryValue.split("-")
        if (attr(0) == "U") {
          val phrase = new NerPhrase(s, t.positionInSection, 1)
          phrase.attr += new NounPhraseEntityType(phrase, attr(1))
          nerSpans += phrase
        } else if (attr(0) == "B") {
          if(t.hasNext) {
            var lookFor = t.next
            while (lookFor.hasNext && lookFor.attr[T].categoryValue.matches("(I|L)-" + attr(1))) lookFor = lookFor.next
            val phrase = new NerPhrase(s, t.positionInSection, lookFor.positionInSection - t.positionInSection)
            phrase.attr += new NounPhraseEntityType(phrase, attr(1))
            nerSpans += phrase
          } else {
            val phrase = new NerPhrase(s, t.positionInSection, 1)
