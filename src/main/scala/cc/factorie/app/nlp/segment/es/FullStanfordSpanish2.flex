...
//  private CoreLabel prevWord;
  var prevWordAfter: StringBuilder = null
  var seenUntokenizableCharacter: Boolean = false

  object UntokenizableOptions {
    val NONE_DELETE = 0
    val FIRST_DELETE = 1
    val ALL_DELETE = 2
    val NONE_KEEP = 3
  ...
  var untokenizable = UntokenizableOptions.FIRST_DELETE
...
//  private boolean strictTreebank3;
   ...
   * typesetting.
   */
  def removeSoftHyphens(in: String): String = {
    val result = in.replaceAll("\u00AD", "")
    if(result.length() == 0) "-" else result
  }
  ...
  def handleEllipsis(tok: String): Object = {
    if (ptb3Ellipsis) {
      getNext(ptb3EllipsisStr, tok)
    } else if (unicodeEllipsis) {
      getNext(unicodeEllipsisStr, tok)
    } else {
      getNext(tok, tok)
    }
  }
  ...
  def normalizeAmp(in: String): String = {
    in.replaceAll("(?i:&amp;)", "&")
  }

  def convertToEl(l: String): String = {
...
//    }
//    return w;
    Array(yychar, yylength())
  }

  def getNormalizedAmpNext(): Object = {
    val txt = yytext()
    if (normalizeAmpersandEntity)
      getNext(normalizeAmp(txt), txt) else getNext()
  }
...
.       { var str = yytext();
          ...
          var msg = String.format("Untokenizable: %s (U+%s, decimal: %s)", yytext(), Integer.toHexString(first).toUpperCase(), Integer.toString(first));
          untokenizable match {
            case UntokenizableOptions.NONE_DELETE =>
              if (invertible) {
            ...
            case UntokenizableOptions.FIRST_KEEP =>
              ...
              return getNext();
            case UntokenizableOptions.ALL_KEEP =>
              LOGGER.warning(msg);
              seenUntokenizableCharacter = true;
              return getNext();
          }
