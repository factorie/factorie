...
trait Mention extends TokenSpan with Entity {
  ...
  def subEntities: Iterable[Entity] = Nil
...
object CorefAffinityDimensionDomain extends EnumDomain {
  val ExactMatch, SuffixMatch, EntityContainsMention, EditDistance2, EditDistance4, NormalizedEditDistance9, NormalizedEditDistance5, Singleton = Value
}
...
object EntityTest {
  ...
  def brainDeadMentionExtraction(doc:Document): Unit = {
    for (i <- 0 until doc.length) {
      ...
      if (doc(i).string.matches("[Hh]e|[Ss]he|[Ii]t") && doc(i).spans.length == 0) new TokenSpan(doc, i, 1)(null) with Mention
      ...
      if (doc(i).isCapitalized && doc(i).spans.length == 0) {
        ...
        new TokenSpan(doc, i, len)(null) with Mention
      }
    }
  }
  ...
  def corefInit(doc:Document): Unit = {
    ...
    for (mention <- doc.spansOfClass[Mention]) mention.attr += new EntityRef(mention, new EntityVariable("NULL"))
  ...
  object EntityRefSampler extends SettingsSampler[EntityRef](new PairwiseModel, null) {
    def settings(entityRef:EntityRef) : SettingIterator = new SettingIterator {
      val mention = entityRef.src.asInstanceOf[Mention]
...
Mr. Obama chose for his speech this hardscrabble town of 4,500 people, about 50 miles south of Kansas City, Kan., where Theodore Roosevelt once laid out the progressive platform he called “the New Nationalism.” Mr. Obama put forth a case for a payroll tax cut and his broader arguments against the Republican economic agenda in what his aides hoped would be viewed as a defining moment.
...
The visit was unusual for its setting in a state that he lost decisively despite his own family roots  — his mother was born in Kansas. The vast majority of his visits as president have been to swing states like Pennsylvania that are expected to play an important role in next year’s election.
