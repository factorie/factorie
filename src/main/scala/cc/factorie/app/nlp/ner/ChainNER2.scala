...
object ChainNer2 extends ChainNer2 {
  ...
  def deSerialize(prefix: String) {
    ...
    println("model.transitionTemplate.weights.length: " + model.transitionTemplate.weights.length)
    println("model.localTemplate.weights.length: " + model.localTemplate.weights.length)
    assertEquals(model.transitionTemplate.weights.length, Conll2003NerDomain.length * Conll2003NerDomain.length)
    BinarySerializer.deserialize(model, modelFile)
    println("model.transitionTemplate.weights.length: " + model2.transitionTemplate.weights.length)
    println("model.localTemplate.weights.length: " + model2.localTemplate.weights.length)


    assert(model2File.exists(), "Trying to load inexisting model file: '" + prefix + "-model2'")
    println("model.transitionTemplate.weights.length: " + model.transitionTemplate.weights.length)
    println("model.localTemplate.weights.length: " + model.localTemplate.weights.length)
    assertEquals(model2.transitionTemplate.weights.length, Conll2003NerDomain.length * Conll2003NerDomain.length)
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (opts.runPlainFiles.wasInvoked) {
      ...
      for (filename <- opts.runPlainFiles.value) {
        ...
        val document = new Document( file.getAbsolutePath, io.Source.fromFile(file).mkString  )
