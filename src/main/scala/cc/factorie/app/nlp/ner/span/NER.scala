...
package cc.factorie.app.nlp.ner.span
...
class TokenSpanSampler(model:TemplateModel, objective:TemplateModel) extends SettingsSampler[Token](model, objective) {
  ...
  def settings(token:Token) = new SettingIterator {
    ...
    for (span <- existingSpans) {
      ...
      for (labelValue <- NerLabelDomain.values; if (labelValue.category != "O"))
...
class NER {
  ...
  def main(args: Array[String]): Unit = {
    ...
    object opts extends DefaultCmdOptions {
      ...
      val verbose =   new CmdOption("verbose", "Turn on verbose output") { override def invoke = NER.this.verbose = true }
  ...
  def train(trainFiles:Seq[String], testFile:String): Unit = {
    ...
    //if (verbose) trainDocuments.take(10).flatMap(x=>x).take(500).foreach(printFeatures _)
    ...
    for (i <- 1 to 11) {
      ...
      if (i % 3 == 0) { if (verbose) println("Removing all spans"); (trainDocuments ++ testDocuments).foreach(_.clearSpans(null)) }
