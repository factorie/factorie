...
package object classify {
  ...
  def labelToFeatures(label:cc.factorie.LabeledCategoricalVariable[String])(implicit model:Model): BinaryFeatureVectorVariable[String] = {
    for (f <- model.factorsOfClass(label, classOf[Factor2[_,_]])) {
      if (f._1.isInstanceOf[BinaryFeatureVectorVariable[String @unchecked]]) return f._1.asInstanceOf[BinaryFeatureVectorVariable[String]]
      else if (f._2.isInstanceOf[BinaryFeatureVectorVariable[String @unchecked]]) return f._1.asInstanceOf[BinaryFeatureVectorVariable[String]]
    }
