...
class MaxEntLikelihoodTrainer(val variance: Double = 10.0, val warmStart: Tensor = null) extends ClassifierTrainer {
  def train[L <: LabeledMutableDiscreteVar[_], F <: DiscreteDimensionTensorVar](il: LabelList[L, F]): Classifier[L] = {
    ...
    if (warmStart != null) cmodel.evidenceTemplate.weightsTensor := warmStart
...
class GeneralClassifierTrainer[L<: LabeledMutableDiscreteVar[_], F<:DiscreteDimensionTensorVar](val trainerConstructor: LogLinearModel[L,F] => Trainer[LogLinearModel[L,F]], val objective: ObjectiveFunctions.MultiClassObjectiveFunction)  {
