...
//  def activeCategories: Seq[T] = vector.activeDomain.toSeq.map(i => domain.dimensionDomain.category(i))
//  //override def toString = vector.activeDomain.map(i => domain.dimensionDomain.getCategory(i).toString+"="+i).mkString(printName+"(", ",", ")")
//  override def toString = vector.activeDomain.map(i => domain.dimensionDomain.category(i).toString).mkString(printName+"(", ",", ")")
//}
//
...
//abstract class FeatureVectorVariableOld[T] extends CategoricalVectorVariable[T]
//
...
//}
//
//abstract class BinaryFeatureVectorVariableOld[T] extends VectorVariable with SparseBinaryCategoricalVectorVar[T] {
//  thisVariable =>
