...
import cc.factorie.util.Implicits._
	...
	class GibbsSampler(model:Model) {
	  ...
	  def incrementIterations = _iterations += 1
		...
		def preVariableSampleHook(v: Variable with MultiProposer): Variable with MultiProposer = v
		...
		protected def xsample(variables: Iterable[Variable with MultiProposer], numIterations: Int, singleSampler: Variable with MultiProposer => Unit): Unit = {
			for (i <- 1 to numIterations) {
				...
				for (variable <- variables.toSeq.shuffle) {
					...
					if (useQueue && Global.random.nextDouble < 0.3 && queue.size > 1) { // TODO Why does > 0 here cause errors?  Bug in queue.size?
						...
						val variables = factor.variables.filter(v => classOf[MultiProposer].isAssignableFrom(v.getClass)).toSeq
						// TODO consider intelligently picking the variable that is causing the problem
						singleSampler(variables(Global.random.nextInt(variables.size)).asInstanceOf[Variable with MultiProposer])
						0
					}
				}
				incrementIterations
      ...
      //val proposals = for (value <- variable.domain) yield { val diff = new DiffList; variable.set(value)(diff); val score = diff.scoreAndUndo(model) / temperature; Proposal1 (score, diff) }
		...
		protected def maximize1(variableToSample: Variable with MultiProposer): DiffList = {
			val variable = preVariableSampleHook(variableToSample)
			case class Proposal(var score: Double, diff:DiffList)
			val proposals = variableToSample.multiPropose(model, null, new DiffList)
