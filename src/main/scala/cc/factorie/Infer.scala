...
trait Infer {
  ...
  def infer(variables:Iterable[Variable], model:Model[Variable], summary:Summary[Marginal] = null): Option[Summary[Marginal]] = None
}
...
object InferDiscrete1 extends Infer {
  ...
  def array(d:DiscreteVariable, model:Model[Variable]): Array[Double] = {
  ...
  def proportions(d:DiscreteVariable, model:Model[Variable]): Proportions1 = new DenseProportions1(array(d, model))
  def marginal[V<:DiscreteVariable](d:V, model:Model[Variable]): DiscreteMarginal1[V] = new DiscreteMarginal1(d, proportions(d, model))
  def apply[V<:DiscreteVariable](varying:Iterable[V], model:Model[Variable]): DiscreteSummary1[V] = {
    val summary = new DiscreteSummary1[V]
    for (v <- varying) summary += new DiscreteMarginal1(v, proportions(v, model))
    summary
  }
  def apply[V<:DiscreteVariable](varying:V, model:Model[Variable]): DiscreteSummary1[V] = {
    val summary = new DiscreteSummary1[V]
...
object InferByGibbsSampling {
  ...
  def withSummary[S<:IncrementableSummary[Marginal]](varying:Iterable[Variable with IterableSettings], model:Model[Variable], summary:S, iterations:Int = 500, burnIn:Int = 100, thinning:Int = 20): S = {
  ...
  def apply[V<:MutableDiscreteVar[_]](varying:Iterable[V], model:Model[Variable], iterations:Int = 500, burnIn:Int = 100, thinning:Int = 20): DiscreteSummary1[V] = {
