...
class MiniBatchExample(val baseExamples: Seq[Example]) extends Example {
  def accumulateValueAndGradient(gradient: WeightsMapAccumulator, value: DoubleAccumulator) {
    baseExamples.foreach(_.accumulateValueAndGradient(gradient, value))
  }
}
...
class StructuredSVMExample[A](labels: A, model: Model with Parameters, loss: Model = HammingLoss, infer: Maximize[A,Model])
  ...
  override def accumulateValueAndGradient(gradient: WeightsMapAccumulator, value: DoubleAccumulator): Unit = {
    if (value != null) {
      val valueAcc = new LocalDoubleAccumulator(0.0)
      super.accumulateValueAndGradient(gradient, valueAcc)
