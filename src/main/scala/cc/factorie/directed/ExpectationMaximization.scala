...
object InferByEM /*extends Infer[(Iterable[DiscreteVar],Iterable[DiscreteVar]),Model]*/ {
  def infer(maximizing:Iterable[DiscreteVar], marginalizing:Iterable[DiscreteVar], model:Model) = {
    val inferencer = new EMInferencer[DiscreteVar,DiscreteVar,Model](maximizing, marginalizing, model, InferByBPTreeSum, Maximize)
    inferencer.process
    inferencer.summary
  }
  def apply(maximize:Iterable[Var], varying:Iterable[DiscreteVariable], model:Model, maximizer:Maximize[Iterable[Var],Model]): Summary = {
    val inferencer = new EMInferencer(maximize, varying, model, InferByBPTreeSum, maximizer)
    inferencer.process
    inferencer.summary
  }
}
//class EMInferencer[V<:Var, M<:Model](val maximizing:Iterable[V], var expectations:DiscreteSummary1[DiscreteVar], val model:M, val expecter:Infer[Iterable[DiscreteVar],M], val maximizer:InferMarginalizing[Iterable[V],M,Summary]) {
//  def eStep: Unit = expectations = expecter.infer(expectations.variables, model)
//  // The "foreach and Seq(v)" below reflect the fact that in EM we maximize the variables independently of each other 
//  def mStep: Unit = maximizer.infer(maximizing, model, expectations).setToMaximize(null) // This "get" will fail if the Maximizer was unable to handle the request
//  def process(iterations:Int): Unit = for (i <- 0 until iterations) { eStep; mStep } // TODO Which should come first?  mStep or eStep?
//  def process: Unit = process(100) // TODO Make a reasonable convergence criteria
//}
//
//object EMInferencer {
//  def apply[V<:Var](maximizing:Iterable[V], varying:Iterable[DiscreteVariable], model: Model, maximizer:InferWithSummary[Iterable[V],Model,Summary] = Maximize, infer: Infer[Iterable[DiscreteVar],Model] = InferByBPTreeSum) =
//    new EMInferencer(maximizing, varying, infer, model, maximizer)
//}
//
//object InferByEM extends InferWithSummary[Iterable[DiscreteVar],DiscreteSummary1[DiscreteVar],Model] {
