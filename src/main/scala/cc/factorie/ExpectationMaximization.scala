...
class EMInferencer[V<:Variable,M<:MeanField](val maximizing:Iterable[V], val meanField:M, val model:Model[Variable], val maximizer: Maximize = Maximize) {
...
object EMInferencer {
  def apply[V<:Variable](maximizing:Iterable[V], varying:Iterable[DiscreteVariable], model:Model[Variable], maximizer:Maximize = Maximize) = new EMInferencer(maximizing, new DiscreteMeanField(varying, model), model, maximizer)
}

object InferByEM extends Infer {
  def apply(maximize:Iterable[Variable], varying:Iterable[DiscreteVariable], model:Model[Variable], maximizer:Maximize = Maximize): DiscreteSummary1[DiscreteVariable] = {
    val meanField = new DiscreteMeanField(varying, model)
    val inferencer = new EMInferencer(maximize, meanField, model, maximizer)
    inferencer.process
    meanField.summary
  }
  override def infer(variables:Iterable[Variable], model:Model[Variable], summary:Summary[Marginal] = null): Option[Summary[Marginal]] = {
    summary match {
