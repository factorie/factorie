...
abstract trait CategoricalVar[A] extends CategoricalVars[A] with DiscreteVar with TypedValue {
  ...
  //type ValueType <: CategoricalValue[VariableType,A]
...
/** A DiscreteVariable whose integers 0...N are associated with an object of type A. 
    @author Andrew McCallum */
@DomainInSubclasses
abstract class CategoricalVariable[A] extends DiscreteVariable with CategoricalVar[A] with MutableTypedValue {
  ...
  def this(initialEntry:A) = { this(); _set(domain.getValue(initialEntry).asInstanceOf[ValueType]) }
...
abstract class CategoricalObservation[A](theEntry:A) extends DiscreteObservation with CategoricalVar[A] {
  ...
  _initializeValue(domain.getValue(theEntry).asInstanceOf[ValueType])
}
...
trait ItemizedObservation[This <: ItemizedObservation[This]] extends CategoricalVar[This] with ConstantValue {
  ...
  override def categoryValue = this // TODO Ug.  Not a great name here.
}
