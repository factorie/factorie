...
object MaximizeGeneratedDiscrete extends Maximize {
  def apply(d:DiscreteVariable, model:Model[Variable]): Unit = {
  ...
  def apply(varying:Iterable[DiscreteVariable], model:Model[Variable]): Unit = for (d <- varying) apply(d, model)
