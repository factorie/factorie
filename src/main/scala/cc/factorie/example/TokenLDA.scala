...
object TokenLDA {
  ...
  def initFeatures(document:Document): Unit = {
    ...
    for (token <- document) {
      ...
      //if (lexicons.size > 0) for (lexicon <- lexicons) if (lexicon.contains(token)) features += "LEX="+lexicon.name
    }
  ...
  def main(args:Array[String]): Unit = {
    ...
    lda.inferTopics(iterations = 40, fitAlphaInterval = 20, diagnosticInterval = 10)
