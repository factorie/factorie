...
object ChainNER1 {
  ...
  def main(args: Array[String]) : Unit = {
    ...
    //trainSentences.foreach(s => s.addNeighboringFeatures(-2,2)) // Add features from 2 next and 2 previous tokens
    trainSentences.foreach(s => s.addNeighboringFeatureConjunctions(List(0), List(-1,0), List(0,1)))
    ...
    {
      //temperature = 0.01
    	override def preProcessHook(label:Label) = if (label.valueIsTruth && Global.random.nextDouble > 0.2) null else label
    ...
    learner.process(trainLabels, 9)
    println("Finished training in "+(System.currentTimeMillis-startTime)/1000.0+" seconds.")
