...
//    optimizer.optimize()
    val examples = sentenceLabels.map(s => new optimize.LikelihoodExample(PosModel, s, InferByBPChainSum))
    val trainer = new optimize.OnlineTrainer(PosModel.weightsSet, new optimize.AROW(PosModel))
