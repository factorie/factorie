...
trait TensorsAccumulator {
  ...
  def accumulate(family: TensorSetKey, t: Tensor): Unit
  def accumulate(family: TensorSetKey, index: Int, value: Double): Unit
  def accumulate(family: TensorSetKey, t: Tensor, factor: Double): Unit
...
class LocalTensorsAccumulator(val tensor: TensorSet) extends TensorsAccumulator {
  def accumulate(family: TensorSetKey, t: Tensor): Unit = tensor(family) += t
  def accumulate(family: TensorSetKey, index: Int, value: Double): Unit = tensor(family)(index) += value
  def accumulate(family: TensorSetKey, t: Tensor, factor: Double) = tensor(family).+=(t, factor)
//  def accumulateOuter(family: Any, t1: Tensor1, t2: Tensor1): Unit = {
...
class SynchronizedTensorsAccumulator(val tensor: TensorSet) extends TensorsAccumulator {
  val l = new LocalTensorsAccumulator(tensor)
  override def accumulate(family: TensorSetKey, t: Tensor): Unit = l.synchronized { l.accumulate(family, t) }
  override def accumulate(family: TensorSetKey, index: Int, value: Double): Unit = l.synchronized { l.accumulate(family, index, value) }
  override def accumulate(family: TensorSetKey, t: Tensor, factor: Double) = l.synchronized { l.accumulate(family, t, factor) }
