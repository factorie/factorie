...
class DenseTensor1(val dim1:Int) extends Tensor1 {
  private var values = new Array[Double](dim1)
  def apply(i:Int) = values(i)
  def update(i:Int, v:Double): Unit = values(i) = v
...
class DenseTensor2(val dim1:Int, val dim2:Int) extends Tensor2 {
  private var values = new Array[Double](dim1*dim2)
  def apply(i:Int, j:Int): Double = values(i*dim2+j)
  def update(i:Int, j:Int, v:Double): Unit = values(i*dim2+j) = v
}
